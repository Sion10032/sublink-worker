...
export class SingboxConfigBuilder extends BaseConfigBuilder {
    constructor(inputString, selectedRules, customRules, baseConfig, lang, userAgent, groupByCountry = false, enableClashUI = false, externalController, externalUiDownloadUrl, singboxVersion = '1.12', includeAutoSelect = true, useProviders = true, mergeUserGroups = false) {
        const resolvedBaseConfig = baseConfig ?? SING_BOX_CONFIG;
        super(inputString, resolvedBaseConfig, lang, userAgent, groupByCountry, includeAutoSelect, useProviders, mergeUserGroups);
        ...
        outbounds.forEach(outbound => {
            if (outbound !== this.t('outboundNames.Node Select')) {
                ...
                if (DIRECT_DEFAULT_RULES.has(outbound)) {
                    selectorMembers = ['DIRECT', 'REJECT', ...selectorMembers.filter(p => p !== 'DIRECT' && p !== 'REJECT')];
                } else if (REJECT_DEFAULT_RULES.has(outbound)) {
                    selectorMembers = ['REJECT', 'DIRECT', ...selectorMembers.filter(p => p !== 'DIRECT' && p !== 'REJECT')];
                }
